import geopandas as gpd
import matplotlib.pyplot as plt
import pandas as pd
import question_1
import question_2
import question_3
import question_4



def main():
    countries = gpd.read_file('/home/data/countries.geojson')
    country_wage_data = pd.read_csv('/home/data/gender-wage-gap-oecd.csv')
    occupations_wage_data = pd.read_csv('/home/data/glassdoor_gender_pay_gap.csv')
    question_3.countries_wage_gap_plot(country_wage_data, countries)
    question_1.occupations_wage_gap_plot(occupations_wage_data)
    #question_4.time_line_chart_plotly(country_wage_data)
    #question_2.education_wage_gap_plot(education_wage_data)


if __name__ == '__main__':
    main()
